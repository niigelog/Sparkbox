...
📖 **[使用教程](使用教程.md)** —— 部署、安装、日常使用、排查，从零开始跟着做。
完整产品方案见 [X收藏插件-开发方案.md](X收藏插件-开发方案.md)。

下面是给开发者看的：架构约束、接口契约、数据库约定。
...
开发时用 `npm run dev` 起 watch，改完代码在扩展页点一下刷新。
...
部署步骤见[使用教程](使用教程.md#第一步部署后端)。开发时也可以不走 Docker 直接跑：

```bash
npm run db:migrate   # 首次：建表
npm run server       # 默认监听 127.0.0.1:7000
```

**监听地址由 `HOST` 环境变量决定，代码里没写死任何 IP** —— 本机开发默认 `127.0.0.1`，容器里 compose 设成 `0.0.0.0`（否则端口映射进不来）。

`.env` 里有两个数据库地址，别搞混：

| 变量 | 谁在用 | 值 |
|---|---|---|
| `APP_POSTGRES_HOST` | 宿主机上的 `npm test` / `npm run db:migrate` | IP |
| `PG_CONTAINER` | 容器内部，走 Docker 网络 | 容器名 |
